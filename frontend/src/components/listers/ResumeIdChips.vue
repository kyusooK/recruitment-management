<template>
    <div>
        <v-list two-line v-if="value.length > 0">
            <v-list-group v-model="isExpansion">
                <template v-slot:activator>
                    <v-list-item-content>
                        <v-list-item-title>ResumeId</v-list-item-title>
                    </v-list-item-content>
                </template>
        ...
        <ResumeId v-if="editMode && tick" offline :isNew="true" :inList="true"
...
<script>
    import ResumeId from '../ResumeId.vue';

    export default {
        name: 'ResumeIdManager',
        components: {
            ResumeId
        },
        props: {
