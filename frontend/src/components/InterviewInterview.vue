<template>
    <v-card outlined>
        ...
        <v-card-title v-if="value._links">
            면접 # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            면접
        </v-card-title >        
        ...
        <v-card-text style="background-color: white;">
            <Date label="resumeId" v-model="value.interviewDate" :editMode="editMode" :inputUI="''"/>
        ...
        methods: {
            ...
            async save(){
                try {
                    ...
                    if(!this.offline) {
                        if(this.isNew) {
                            temp = await axios.post(axios.fixUrl('/interviews'), this.value)
                        } else {
